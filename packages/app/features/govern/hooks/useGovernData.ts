...
export const canVoteThisEpoch = (lastVoted: bigint): boolean => {
  ...
  const epochStart =
    (now / BigInt(EPOCH_DURATION_SECONDS)) * BigInt(EPOCH_DURATION_SECONDS);
...
export function useGovernData(userAddress?: Address) {
  ...
  const { data: rawStrategiesData, refetch: refetchStrategiesData } =
    useReadContract({
      address: LSG_ADDRESSES.lsgMulticall as Address,
      abi: LSG_MULTICALL_ABI,
  ...
  const strategiesData = useMemo(() => {
    ...
    return (rawStrategiesData as unknown as StrategyData[]).filter(
      (s) => s.isAlive
    );
  }, [rawStrategiesData]);
  ...
  const { data: donutAllowance, refetch: refetchAllowance } = useReadContract({
    ...
    args: [
      ...
      LSG_ADDRESSES.governanceToken as Address,
    ],
    chainId: base.id,
  ...
  const totalPendingRewards = useMemo(() => {
    ...
    const rewardMap = new Map<
      string,
      { token: Address; decimals: number; amount: bigint }
    >();
