...
import { LSG_ADDRESSES, VOTER_ABI } from "@/config/govern-constants";
...
export function useVoting(userAddress?: Address, onSuccess?: () => void) {
  ...
  // Wait for receipts
  const { data: voteReceipt, isLoading: isVoteConfirming } =
    useWaitForTransactionReceipt({
      hash: voteTxHash,
      chainId: base.id,
    });

  const { data: resetReceipt, isLoading: isResetConfirming } =
    useWaitForTransactionReceipt({
      hash: resetTxHash,
      chainId: base.id,
    });

  const { data: claimReceipt, isLoading: isClaimConfirming } =
    useWaitForTransactionReceipt({
      hash: claimTxHash,
      chainId: base.id,
    });
  ...
  const totalVoteWeight = Object.values(voteWeights).reduce(
    (sum, w) => sum + w,
    0
  );
  ...
  const handleVote = useCallback(async () => {
    ...
    setTxStep("voting");
    try {
      const strategies = Object.keys(voteWeights).filter(
        (s) => voteWeights[s] > 0
      ) as Address[];
