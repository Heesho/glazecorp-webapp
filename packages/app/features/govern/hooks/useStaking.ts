...
export function useStaking(
  ...
  const { data: stakeReceipt, isLoading: isStakeConfirming } =
    useWaitForTransactionReceipt({
      hash: stakeTxHash,
      chainId: base.id,
    });

  const { data: delegateReceipt, isLoading: isDelegateConfirming } =
    useWaitForTransactionReceipt({
      hash: delegateTxHash,
      chainId: base.id,
    });

  const { data: resetVotesReceipt, isLoading: isResetVotesConfirming } =
    useWaitForTransactionReceipt({
